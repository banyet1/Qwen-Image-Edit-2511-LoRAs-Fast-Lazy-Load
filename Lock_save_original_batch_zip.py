    ...
    function checkResult() {
        ...
        try {
            ...
            if (data.status === 'ok' && data.images && data.images.length > 0) {
                lastProcessed = val;
                const hasUserImages = Array.isArray(window.__uploadedImages) && window.__uploadedImages.length > 0;
                ...
                if (window.__showToast) {
                    if (hasUserImages) window.__showToast('Example prompt/model loaded — uploaded images preserved', 'info');
                    else window.__showToast('Example loaded \u2014 ' + data.images.length + ' image(s)', 'info');
                }
